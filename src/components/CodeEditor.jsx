...
export default function CodeEditor({ code }) {
  // default template (if AI doesn't output any HTML)
...
<html>
  ...
  // update editor when AI returns content
  useEffect(() => {
    if (code && code.trim()) {
      // If the AI returns raw HTML, use it; otherwise try to wrap in body
      const trimmed = code.trim();
      if (trimmed.startsWith("<")) setEditorHtml(trimmed);
      else {
        // wrap plain text into a simple HTML page
        setEditorHtml(
          `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Preview</title></head><body><pre>${escapeHtml(
            trimmed
          )}</pre></body></html>`
        );
      }
